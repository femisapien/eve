...
export interface SlackHandle {
   ...
   */
  request(operation: string, body: object): Promise<SlackApiResponse>;
...
export function buildSlackWorkspaceHandle(input: {
  ...
  return {
    teamId: input.teamId,
    request: (operation, body) => input.transport.call(operation, asSlackApiBody(body), context),
  };
}
...
export function buildSlackBinding(input: {
  ...
  const request: SlackHandle["request"] = (operation, body) =>
    input.transport.call(operation, asSlackApiBody(body), context);
