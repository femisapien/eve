...
function createPane(
  ...
  const scheduleSave = () => {
    if (!saveTimer && persistence) saveTimer = setTimeout(flush, 250);
  };
  ...
  const connect = () => {
    // The workspace owns this connection. View subscriptions never restart it.
    ...
    void (async () => {
      ...
      try {
        ...
        publish();
        const response = await fetchStream(subagentStreamPath(session, state.cursor), {
          cache: "no-store",
        ...
        reconnect = !state.terminal && !active.signal.aborted;
      } catch (cause) {
...
/** Workspace-local subscriptions; pane visibility does not own their lifetime. */
export function createSubagentPaneCache(fetchStream?: StreamFetch, storage?: SubagentStorage) {
  ...
  return {
    ...
    get(session: SubagentSession): SubagentPaneState {
      ...
      const state =
        entries.get(key) ??
        createPane(session, fetcher, storage ? { storage, owner: ownerReady } : undefined);
      entries.delete(key);
      entries.set(key, state);
      if (entries.size > 20) {
        const oldest = [...entries].find(
          ([, entry]) => entry !== state && entry.cursor > 0 && !entry.hasSubscribers(),
        );
        if (oldest) {
          oldest[1].dispose();
