...
export function SubagentActivity({
  ...
  if (disconnected && progress.endedAt === undefined) return <span>Disconnected</span>;
  ...
  if (progress.phase === "failed") return <span>Failed after {elapsed}</span>;
  if (progress.phase === "cancelled") return <span>Cancelled after {elapsed}</span>;
  return (
    <>
