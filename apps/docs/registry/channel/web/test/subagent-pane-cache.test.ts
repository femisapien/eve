...
test("pane switches keep one stream per child and continue receiving while hidden", async () => {
  ...
  try {
    const a = cache.get(child("a"));
    const closeA = a.subscribe(() => {});
    await new Promise((resolve) => setTimeout(resolve, 0));
    streams.get(requests[0])!.enqueue(
    ...
    const closeB = cache.get(child("b")).subscribe(() => {});
    streams.get(requests[0])!.enqueue(
      new TextEncoder().encode(
        JSON.stringify({
          type: "message.completed",
          ...
          meta: { id: "2", at: "2026-09-20T12:01:00Z" },
        }) + "\n",
      ),
    );
    await new Promise((resolve) => setTimeout(resolve, 25));
    closeB();
    for (let i = 0; i < 3; i++) {
      cache.get(child("a")).subscribe(() => {})();
      cache.get(child("b")).subscribe(() => {})();
    }
    assert.equal(requests.length, 2, "one initial request per child, none on reopening");
    assert.equal(a.cursor, 2);
    assert.match(JSON.stringify(a.getSnapshot().data.messages), /updated while hidden/);
  } finally {
    cache.clear();
  }
  assert.equal(
...
test("stream renewal resumes at the retained cursor without replaying history", async () => {
  ...
  try {
    ...
    await new Promise((resolve) => setTimeout(resolve, 25));
    a.subscribe(() => {})();
    assert.equal(requests.length, 1, "reopening during stream renewal must not trigger a request");
    a.retry();
...
test("a fresh client restores the projection before subscribing at its saved cursor", async () => {
  const storage = disk();
  const first = createSubagentPaneCache(async () => new Response(eventLine("a", "Hello")), storage);
  ...
  const second = createSubagentPaneCache(async (path) => {
    requests.push(path);
    return new Response(eventLine("b", " world"));
  }, storage);
...
test("storage failure falls back to the durable stream without losing functionality", async () => {
  ...
  const cache = createSubagentPaneCache(
    async (p) => {
      path = p;
      return new Response(eventLine("a", "answer"));
    },
    {
      ...storage,
      async load() {
...
test("a restored done row stays done while an unsaved historical tail catches up", async () => {
  const storage = disk();
  const first = createSubagentPaneCache(async () => new Response(eventLine("a", "saved")), storage);
  first.setOwner("alice");
...
test("a completed turn stays completed through waiting and late child notifications", async () => {
  const events = ["turn.started", "turn.completed", "session.waiting", "subagent.completed"]
    ...
    .join("");
  const cache = createSubagentPaneCache(async () => new Response(events));
  try {
    const entry = cache.get(child("a"));
    entry.connect();
    await tick();
