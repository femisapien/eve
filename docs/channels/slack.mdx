...
Pass the same [`api`](#point-the-channel-at-a-different-slack-api-host) the
channel was given. `callSlackApi` has no channel to inherit it from, so
omitting it sends the call to `https://slack.com/api/` with whatever token you
supplied — the one way left to reach the real workspace from a deployment
pointed at a stand-in. Leaving `apiBaseUrl` `undefined` keeps Slack's own host,
so a single `process.env` read works in both deployments.

### Point the channel at a different Slack API host

`api` overrides the transport every outbound Slack Web API call uses. Every
field is optional; omit `api` and the channel talks to `https://slack.com/api/`
through the global `fetch`, exactly as before.
...
`apiBaseUrl` must be an absolute `http:` or `https:` URL carrying no query
string or fragment: method names are appended by relative URL resolution, which
keeps only the origin and the directory part of the path, so anything trailing
it would be silently dropped. A base without a trailing slash is normalized to
one, so `https://sim.example/api` and `https://sim.example/api/` both send
`chat.postMessage` to `https://sim.example/api/chat.postMessage`. An empty
string is rejected rather than treated as "unset", so
`apiBaseUrl: process.env.FAKE_SLACK_URL ?? ""` fails loudly instead of reaching
the real workspace with the real token.

`fileBaseUrl` names where the stand-in serves its own attachments, and
defaults to `apiBaseUrl`. A download URL counts as the stand-in's when it
starts with this base, so set it to the host root when files are served from
paths outside the API base:
...
It is validated like `apiBaseUrl` and normalized the same way. Slack's own file
hosts (`files.slack.com` and Enterprise Grid) are always downloadable and are
not affected by it; every other URL still passes through to the model provider
unchanged.

`api.fetch` replaces the global `fetch` for requests to `apiBaseUrl` and to
`fileBaseUrl` — useful for adding a per-call header, such as a
deployment-protection bypass, or for instrumenting the calls. It is confined to
those two bases on purpose: a wrapper usually attaches credentials meant for
the stand-in, which have no business on a request to Slack's own hosts. That
includes the second leg of a file upload, whose URL
`files.getUploadURLExternal` chooses: it goes through `api.fetch` only when it
lands on the `apiBaseUrl` origin, and with no `apiBaseUrl` set it never does. Inbound webhook verification is untouched either way:
that still uses `credentials.signingSecret` / `SLACK_SIGNING_SECRET`.
...
Downloads are restricted to Slack's own file hosts, plus URLs under a configured [`api.fileBaseUrl`](#point-the-channel-at-a-different-slack-api-host) (which falls back to `api.apiBaseUrl`). Any other URL is passed through to the model provider unchanged.
