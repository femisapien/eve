...
Listing skills and loading their markdown do not require a sandbox, including dynamic skills with supporting files. eve reads static instructions from the compiled agent and dynamic instructions from durable session state. Supporting files are staged when a tool or hook accesses the sandbox, including through `ctx.getSkill(id).file(...)` or a built-in filesystem tool. Builds may also prewarm sandbox templates that contain static package files.

Supporting files are available under `$HOME/.agents/skills/<skill>/`, with `/workspace/skills/<skill>/` as the fallback when `$HOME` is unavailable. Sibling references inside `SKILL.md`, such as `references/checklist.md`, are relative to the directory containing that specific `SKILL.md`.
...
eve preserves each resolved package, including its supporting files, across durable workflow steps. An unchanged package is not rewritten on refresh. When a resolver changes a package, eve replaces its staged files, removing files omitted from the new result. A removed dynamic skill is no longer loadable; if it overrode an authored skill, the authored skill becomes available again.

Dynamic packages can contain up to 128 files, including `SKILL.md`, with at most 256 KiB per file. All active dynamic packages together must fit within 1 MiB of serialized session state, including base64 encoding and instruction text. An oversized result fails resolution before changing the active packages. Keep large assets outside the skill package and read them through a tool when needed.
