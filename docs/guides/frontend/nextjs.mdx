...
Improve generated Web Chat link and code contrast, simplify code blocks, and keep a compact composer visible while scrolling.

Add consistent copying for tool commands, parameters, output and errors. Preserve disclosure choices during streaming and reduce repeated thinking shimmer.

Preserve repeated chat messages, follow new submissions, and derive parent activity independently of children. Disable input when the server is unreachable and recover on reconnection.

Add local session history ordered by recent messages, metadata tooltips, responsive navigation, and cached session switching.
