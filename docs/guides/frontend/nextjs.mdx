...
Local development reads sessions from the app's local eve workflow storage.
For deployed, signed-in history, select Sign in with Vercel and configure
...
The generated adapter derives ownership from the verified Vercel account subject
in the Better Auth session. Deployment OIDC credentials are separate from browser
identity. The base Web Chat scaffold deliberately leaves its production viewer
...
indexed sessions and inspect children linked by their parent's durable events.
Session creation and index writes are not transactional together: a database
outage may leave an unlisted session or stale activity metadata. Existing sessions
